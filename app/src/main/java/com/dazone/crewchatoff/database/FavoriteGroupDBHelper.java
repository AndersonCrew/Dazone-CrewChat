...
public class FavoriteGroupDBHelper {
    ...
    public static ArrayList<FavoriteGroupDto> getFavoriteGroup() {
        ...
        Cursor cursor = resolver.query(AppContentProvider.GET_FAVORITE_GROUP_CONTENT_URI, columns, null, null, null);
            ...
            if (cursor.getCount() > 0) {
                try {
                    while (cursor.moveToNext()) {
                        ...
                        boolean flag = true;
                        for (FavoriteGroupDto obj : favoriteGroups) {
                            if (obj.getGroupNo() == groupNo) {
                                flag = false;
                                break;
                            }
                        }
    ...
    public static boolean isExist(FavoriteGroupDto group) {
        String[] columns = new String[]{"*"};
        ContentResolver resolver = CrewChatApplication.getInstance().getApplicationContext().getContentResolver();
        Cursor cursor = resolver.query(AppContentProvider.GET_FAVORITE_GROUP_CONTENT_URI, columns, GROUP_NO + "=" + group.getGroupNo(), null, null);
